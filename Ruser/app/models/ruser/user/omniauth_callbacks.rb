  ...
  module OmniauthCallbacks
    ["github","google","twitter","douban"].each do |provider|
      define_method "find_or_create_for_#{provider}" do |response|
        ...
        if user =  Ruser::User.where("authorizations.provider" => provider , "authorizations.uid" => uid).first
          user
        elsif user =  Ruser::User.find_by_email(data["email"])
          user.bind_service(response)
          user
        else
          user = Ruser::User.new_from_provider_data(provider, uid, data)
    ...
    def new_from_provider_data(provider, uid, data)
      Ruser::User.new do |user|
        ...
        if Ruser::User.where(:login => user.login).count > 0 || user.login.blank?
